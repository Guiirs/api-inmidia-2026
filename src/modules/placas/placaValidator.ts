...
export const placaValidationRules: ValidationChain[] = [
    ...
    // Compatibilidade: aceita 'regiaoId' (front atual) e 'regiao' (legado)
    body().custom((_value, { req }) => {
        const regiaoId = req.body?.regiaoId || req.body?.regiao;
        if (!regiaoId) throw new Error('A regi?o ? obrigat?ria.');

        const mongoIdRegex = /^[a-f\d]{24}$/i;
        if (!mongoIdRegex.test(String(regiaoId))) {
            throw new Error('ID da regi?o inv?lido.');
        }

        req.body.regiaoId = String(regiaoId);
        return true;
    })
];
