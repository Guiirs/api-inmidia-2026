 ...
 */
async function runScript(scriptRelativePath: string, options: RunScriptOptions = {}) {
  await ensureLogsDir();
  ...
  if (ext === '.js') {
    ...
    try {
      ...
      const requireErrMsg = requireErr instanceof Error ? requireErr.message : String(requireErr);
      logger.warn(`[ScriptRunner] require() falhou para ${scriptPath}, fallback para spawn: ${requireErrMsg}`);
