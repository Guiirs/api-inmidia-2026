/**
 * Relatorio Service (OLD)
 * DEPRECADO: Use services/relatorio.service.ts
 */
...
class RelatorioService {
     ...
     */
    async getDashboardSummary(empresa_id: string) {
        ...
        try {
            ...
            const totalPlacasPromise = Placa.countDocuments({ empresaId: empresaObjectId });
            ...
            logger.error(`[RelatorioService] Erro ao gerar Dashboard Summary: ${this.getErrorMessage(error)}`, { stack: this.getErrorStack(error) });
            throw new AppError(`Erro interno ao buscar resumo do dashboard: ${this.getErrorMessage(error)}`, 500);
        }
    }
     ...
     */
    async placasPorRegiao(empresa_id: string) {
        ...
        try {
            ...
            logger.error(`[RelatorioService] Erro ao buscar placasPorRegiao: ${this.getErrorMessage(error)}`, { stack: this.getErrorStack(error) });
            throw new AppError(`Erro interno ao buscar placas por região: ${this.getErrorMessage(error)}`, 500);
        }
    }
     ...
     */
    async ocupacaoPorPeriodo(empresa_id: string, dataInicio: Date, dataFim: Date) {
     ...
     * @returns {void}
     */
    async generateOcupacaoPdf(reportData: any, dataInicio: Date, dataFim: Date, res: Response) {
        logger.info('[RelatorioService] Iniciando geração do PDF de Ocupação (Nativo com PDFKit).');

        // Helper para formatar a data
        const formatPdfDate = (date: Date) => `${String(date.getUTCDate()).padStart(2, '0')}/${String(date.getUTCMonth() + 1).padStart(2, '0')}/${date.getUTCFullYear()}`;
        ...
        reportData.ocupacaoPorRegiao.forEach((r: any) => {
        ...
        reportData.novosAlugueisPorCliente.forEach((c: any) => {
             if (currentY > 720) { 
                doc.addPage(); 
                currentY = doc.page.margins.top;
            }
